...
class Simulation:
    ...
    def __init__(self, population_size=1000, generations=2000, budgets=12, elitism_rate=0.10):
    ...
    def run(self):
        self.points_mat = self.cards_ref[:, -20:-10].astype(int)
        self.cashback_mat = self.cards_ref[:, -10:].astype(int) 
        self.annual_fee_mat = self.cards_ref[:, 2].astype(int)
        self.anniv_points_mat = self.cards_ref[:, 3].astype(int)
        self.offer_points_mat = self.cards_ref[:, 4].astype(int)
        self.offer_cash_mat = self.cards_ref[:, 5].astype(int)
    ...
    def run_sim_for_consoomer(self, c):
        points = np.array([np.diag(self.points_mat[c.purchases, :])]).reshape(1, 10, 1)
        cashback = np.array([np.diag(self.cashback_mat[c.purchases, :])]).reshape(1, 10, 1)

        cashback, other_cb = self.calc_bofa_rewards(cashback, c)

        points = np.sum(points * self.year, axis=1)
        cashback = np.sum(cashback * self.year, axis=1)

        bonus_cb = np.zeros((len(self.consoomers)))
        ...
        return np.sum(points + cashback, axis=1) + annual_fee + bonus_p + bonus_cb + other_cb
    
    def calc_bofa_rewards(self, cashback, c):
        ...
        if 30 in c.cards:
            ...
            bofa_cc_categories = c.purchases == 999 #change this to the index of the bank of america card
            quarter_sum = np.sum(self.year, axis=1)
            travel_sum = np.sum(quarter_sum * np.array([1, 1, 0, 1, 0, 0, 0, 0, 0, 0])*bofa_cc_categories)
            max_cat_not_trav = np.argmax(quarter_sum*np.array([0, 0, 0, 0, 0, 1, 0, 1, 1, 0]) * bofa_cc_categories)
            if travel_sum < quarter_sum[max_cat_not_trav]:
                ...
                if travel_sum < bofa_max_cc_bonus_cb:
                    ...
                    other_cb += (bofa_max_cc_bonus_cb * 0.03) + (travel_sum - 2500) * 0.01
                    for i in [0, 1, 3]:
                        cashback[0, i, 0] = 0
        return cashback, other_cb

    def load_cards(self):
        dat = pd.read_csv("C:/Users/EvanChase/Documents/Repos/credit-card-calculator/cards.csv")
        self.cards_ref = np.asarray(dat.iloc[0:31, 2:].to_numpy())
